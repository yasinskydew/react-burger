import ordersReducer, { setIsOrderModalOpenReducer } from './orders';
...
describe('orders reducer', () => {
  it('returns the initial state', () => {
    expect(ordersReducer(undefined, { type: '' })).toEqual(getInitialState());
  });

  it('handles setIsOrderModalOpenReducer', () => {
    const state = ordersReducer(getInitialState(), setIsOrderModalOpenReducer(true));
    expect(state.isOrderModalOpen).toBe(true);
  });

  it('handles wsConnected', () => {
    const state = ordersReducer(getInitialState(), wsConnected());
    expect(state.wsConnected).toBe(true);
    expect(state.wsError).toBeNull();
  });

  it('handles wsDisconnected', () => {
    const prevState = { ...getInitialState(), wsConnected: true };
  ...
  it('handles wsOrdersReceived', () => {
    ...
    const state = ordersReducer(getInitialState(), wsOrdersReceived(payload));
